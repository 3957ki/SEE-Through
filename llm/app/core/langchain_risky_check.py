...
아래의 음식 목록을 분석하여 **사용자의 알레르기 정보, 질병 정보, 나이 정보에 따라 섭취 시 위험할 수 있는 음식**에 대해  
의학적으로 구체적인 이유와 함께 경고 메시지를 작성하세요. (100자정도)

💡 반드시 다음 원칙을 지키세요:

1. **사용자의 알레르기 목록에 포함된 음식**은, 알레르기 반응의 구체적인 증상이나 기전(예: 히스타민 분비, 아나필락시스 등)을 언급합니다.
...
3. **사용자의 나이 정보에 따른 주의**도 반영합니다:
   - 만 19세 미만: 술, 카페인, 자극적인 음식 주의
   - 고령자(만 65세 이상): 고염분/고지방/질긴 음식, 소화 어려운 음식 주의
4. **전문적이고 다양하게 표현하며, 절대 단순 문장 반복 금지**
5. 사용자의 입력에 없는 정보는 절대 기반으로 삼지 마세요.
6. 메시지는 실제 위험성을 기반으로 하되, 과도한 불안 조장은 피하세요.
...
def analyze_risky_foods_with_comments(
    ...
    if age <= 12:
        child_friendly_clause = """
    🧒 사용자가 어린이이므로, **친절하고 이해하기 쉬운 반말**로 설명해주세요.
